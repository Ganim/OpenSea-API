...
export const getTimelineQuerySchema = z
  ...
  .refine((data) => data.contactId || data.customerId || data.dealId, {
    message:
...
export const timelineItemResponseSchema = z.object({
  ...
  date: z.coerce
    .date()
    .describe('Item date (performedAt for activity, createdAt for event)'),
  title: z.string().describe('Item title'),
  activityType: z
    .string()
    .nullable()
    .optional()
    .describe('Activity type (NOTE, CALL, etc.)'),
  eventType: z
    .string()
    .nullable()
    .optional()
    .describe('Timeline event type (STAGE_CHANGE, etc.)'),
  metadata: z
