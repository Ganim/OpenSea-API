...
describe('Update Subtask (E2E)', () => {
  ...
  it('should update a subtask', async () => {
    ...
    const subtask = await createTaskCard(board.id, columns[0].id, userId, {
      title: 'Subtarefa Original',
      parentCardId: card.id,
    });

    const response = await request(app.server)
      .patch(
        `/v1/tasks/boards/${board.id}/cards/${card.id}/subtasks/${subtask.id}`,
      )
      .set('Authorization', `Bearer ${token}`)
      .send({ title: 'Subtarefa Atualizada' });

    expect(response.status).toBe(200);
    expect(response.body.subtask.title).toBe('Subtarefa Atualizada');
  });
});
