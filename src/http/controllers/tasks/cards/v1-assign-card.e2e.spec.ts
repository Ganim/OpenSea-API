...
describe('Assign Card (E2E)', () => {
  ...
  it('should assign a card to a user', async () => {
    ...
    const response = await request(app.server)
      .patch(`/v1/tasks/boards/${board.id}/cards/${card.id}/assign`)
      .set('Authorization', `Bearer ${token}`)
      .send({ assigneeId: userId });

    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('card');
    expect(response.body.card.assigneeId).toBe(userId);
  });
});
