...
describe('List Card Integrations (E2E)', () => {
  let tenantId: string;
  ...
  it('should list integrations of a card', async () => {
    ...
    await request(app.server)
      ...
      .send({
        ...
        entityLabel: 'arquivo-list.pdf',
      });

    const response = await request(app.server)
      .get(`/v1/tasks/boards/${board.id}/cards/${card.id}/integrations`)
      .set('Authorization', `Bearer ${token}`);
  ...
  it('should return empty array for card with no integrations', async () => {
    const { token, user } = await createAndAuthenticateUser(app, { tenantId });
    const userId = user.user.id;
