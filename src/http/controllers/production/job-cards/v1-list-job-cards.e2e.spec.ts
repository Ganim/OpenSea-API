...
describe('List Job Cards (E2E)', () => {
  ...
  it('should list job cards', async () => {
    const response = await request(app.server)
      .get('/v1/production/job-cards')
      .query({ productionOrderId })
      .set('Authorization', `Bearer ${token}`);

    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('jobCards');
    expect(Array.isArray(response.body.jobCards)).toBe(true);
    expect(response.body.jobCards.length).toBeGreaterThanOrEqual(1);
  });
});
