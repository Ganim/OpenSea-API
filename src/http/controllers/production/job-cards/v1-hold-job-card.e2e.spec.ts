...
describe('Hold Job Card (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    // Start the job card first so it can be put on hold
  ...
  it('should put a job card on hold', async () => {
    const response = await request(app.server)
      .post(`/v1/production/job-cards/${jobCardId}/hold`)
    ...
    expect(response.body.jobCard.status).toBe('ON_HOLD');
  });
});
