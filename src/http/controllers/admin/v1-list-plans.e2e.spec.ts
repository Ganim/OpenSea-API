...
describe('List Plans (E2E)', () => {
  ...
  it('should list all plans', async () => {
    ...
    expect(response.body.plans.length).toBeGreaterThanOrEqual(1);
    expect(response.body.plans[0]).toHaveProperty('id');
    expect(response.body.plans[0]).toHaveProperty('name');
    expect(response.body.plans[0]).toHaveProperty('tier');
  });
