...
import { makeCreateCompanyAddressUseCase } from '@/use-cases/admin/company-addresses/factories/make-company-addresses';
...
export async function createCompanyAddressController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: 'POST',
    url: '/v1/admin/companies/:companyId/addresses',
    preHandler: [
      verifyJwt,
      verifyTenant,
      createPermissionMiddleware({
        permissionCode: PermissionCodes.ADMIN.COMPANY_ADDRESSES.CREATE,
        resource: 'company-addresses',
      }),
    ],
    schema: {
      tags: ['Admin - Company Addresses'],
