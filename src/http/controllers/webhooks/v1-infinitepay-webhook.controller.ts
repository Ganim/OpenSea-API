...
export async function v1InfinitepayWebhookController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: 'POST',
    url: '/v1/webhooks/infinitepay',
    // NO JWT or tenant middleware — public endpoint called by InfinitePay
    schema: {
      tags: ['Webhooks'],
      summary:
        'Receive InfinitePay webhook events (payment confirmed, failed, etc.)',
    ...
    handler: async (request, reply) => {
      ...
      // Extract relevant headers for signature validation
      ...
      try {
        ...
        console.error('[InfinitePayWebhook] Error processing webhook:', error);
      }
