...
describe('Delete Folder (E2E)', () => {
  ...
  it('should delete a folder', async () => {
    ...
    const response = await request(app.server)
      .delete(`/v1/storage/folders/${folderId}`)
      .set('Authorization', `Bearer ${token}`);

    expect(response.status).toBe(204);

    const deleted = await prisma.storageFolder.findUnique({
      where: { id: folderId },
    });
  ...
  it('should return 404 for non-existent folder', async () => {
    ...
    const response = await request(app.server)
      .delete('/v1/storage/folders/00000000-0000-0000-0000-000000000000')
      .set('Authorization', `Bearer ${token}`);
