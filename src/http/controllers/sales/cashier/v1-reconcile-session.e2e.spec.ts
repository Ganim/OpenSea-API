...
describe('Reconcile Cashier Session (E2E)', () => {
  let tenantId: string;
  ...
  it('should reconcile a closed session (200)', async () => {
    ...
    const response = await request(app.server)
      .patch(`/v1/sales/cashier/sessions/${sessionId}/reconcile`)
      .set('Authorization', `Bearer ${token}`);

    expect(response.status).toBe(200);
    expect(response.body.cashierSession).toBeDefined();
    expect(response.body.cashierSession.status).toBe('RECONCILED');
  });
});
