...
describe('Get Landing Page By ID (E2E)', () => {
  let tenantId: string;
  ...
  it('should get landing page by id', async () => {
    ...
    const createResponse = await request(app.server)
      ...
      .send({
        title: `Get By ID Page ${timestamp}`,
        slug: `get-by-id-${timestamp}`,
    ...
    const response = await request(app.server)
      .get(`/v1/sales/landing-pages/${landingPageId}`)
      .set('Authorization', `Bearer ${token}`);

    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('landingPage');
    expect(response.body.landingPage.id).toBe(landingPageId);
  });
