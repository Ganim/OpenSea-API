...
import { makeActivateWorkflowUseCase } from '@/use-cases/sales/workflows/factories/make-activate-workflow-use-case';
import type { FastifyInstance } from 'fastify';
import type { ZodTypeProvider } from 'fastify-type-provider-zod';
import { z } from 'zod';

export async function activateWorkflowController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: 'PATCH',
    url: '/v1/sales/workflows/:id/activate',
    preHandler: [
      verifyJwt,
      verifyTenant,
    ...
    handler: async (request, reply) => {
      ...
      try {
        const useCase = makeActivateWorkflowUseCase();
