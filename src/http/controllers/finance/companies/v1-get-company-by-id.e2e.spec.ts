...
describe('Get Company By ID - Finance read-only (E2E)', () => {
  let tenantId: string;
  ...
  it('should get company by id with finance.companies.read permission', async () => {
    ...
    const { company } = await createCompanyUseCase.execute({
      tenantId,
      legalName: `Finance Detail Company ${Date.now()}`,
    ...
    const response = await request(app.server)
      .get(`/v1/finance/companies/${companyId}`)
  ...
  it('should return 404 for nonexistent company', async () => {
    ...
    const response = await request(app.server)
      .get('/v1/finance/companies/00000000-0000-0000-0000-000000000000')
      .set('Authorization', `Bearer ${token}`);
