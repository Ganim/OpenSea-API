...
import { makeApprovePaymentOrderUseCase } from '@/use-cases/finance/payment-orders/factories/make-approve-payment-order-use-case';
...
export async function approvePaymentOrderController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: 'POST',
    url: '/v1/finance/payment-orders/:id/approve',
    preHandler: [
      verifyJwt,
      verifyTenant,
    ...
    schema: {
      ...
      summary: 'Approve a payment order and execute the payment',
      security: [{ bearerAuth: [] }],
    ...
    handler: async (request, reply) => {
      ...
      try {
        const useCase = makeApprovePaymentOrderUseCase();
