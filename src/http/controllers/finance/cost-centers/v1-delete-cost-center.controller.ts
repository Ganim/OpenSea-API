...
import { makeDeleteCostCenterUseCase } from '@/use-cases/finance/cost-centers/factories/make-delete-cost-center-use-case';
import type { FastifyInstance } from 'fastify';
import type { ZodTypeProvider } from 'fastify-type-provider-zod';
import { z } from 'zod';

export async function deleteCostCenterController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        const useCase = makeDeleteCostCenterUseCase();
        await useCase.execute({ tenantId, id });

        await logAudit(request, {
          message: AUDIT_MESSAGES.FINANCE.COST_CENTER_DELETE,
          entityId: id,
          placeholders: { userName, costCenterName: oldData.costCenter.name },
