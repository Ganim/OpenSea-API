...
import { makeCancelFiscalDocumentUseCase } from '@/use-cases/fiscal/documents/factories/make-cancel-fiscal-document-use-case';
import type { FastifyInstance } from 'fastify';
import type { ZodTypeProvider } from 'fastify-type-provider-zod';
import { z } from 'zod';

export async function cancelDocumentController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: 'POST',
    url: '/v1/fiscal/documents/:id/cancel',
    preHandler: [
      verifyJwt,
      verifyTenant,
    ...
    schema: {
      tags: ['Fiscal'],
      summary: 'Cancel an authorized fiscal document',
    ...
    handler: async (request, reply) => {
      ...
      try {
        const useCase = makeCancelFiscalDocumentUseCase();
        ...
        await logAudit(request, {
          message: AUDIT_MESSAGES.FISCAL.DOCUMENT_CANCEL,
