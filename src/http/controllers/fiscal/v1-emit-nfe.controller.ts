...
import { makeEmitNFeUseCase } from '@/use-cases/fiscal/documents/factories/make-emit-nfe-use-case';
import type { FastifyInstance } from 'fastify';
import type { ZodTypeProvider } from 'fastify-type-provider-zod';
import { z } from 'zod';

export async function emitNFeController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    ...
    schema: {
      tags: ['Fiscal'],
      summary: 'Emit an NF-e (Nota Fiscal Eletronica)',
    ...
    handler: async (request, reply) => {
      ...
      try {
        const useCase = makeEmitNFeUseCase();
        ...
        await logAudit(request, {
          message: AUDIT_MESSAGES.FISCAL.NFE_EMIT,
