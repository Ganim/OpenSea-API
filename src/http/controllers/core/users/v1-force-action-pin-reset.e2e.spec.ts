...
describe('Force Action PIN Reset (E2E)', () => {
  let tenantId: string;
  ...
  it('should force action PIN reset with correct schema', async () => {
    ...
    const response = await request(app.server)
      .post(`/v1/users/${targetUserResponse.user.id}/force-action-pin-reset`)
  ...
  it('should return 404 for non-existent user', async () => {
    ...
    const response = await request(app.server)
      .post('/v1/users/00000000-0000-0000-0000-000000000000/force-action-pin-reset')
      .set('Authorization', `Bearer ${adminToken}`);
