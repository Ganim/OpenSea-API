...
describe('Toggle My Auth Link (E2E)', () => {
  let tenantId: string;
  ...
  it('should toggle auth link status', async () => {
    ...
    // Create a second auth link so we can toggle it
    await request(app.server)
      .post('/v1/me/auth-links')
      .set('Authorization', `Bearer ${token}`)
      .send({
        provider: 'CPF',
        identifier: '222.333.444-55',
  ...
  it('should return 401 without token', async () => {
    const response = await request(app.server)
      .patch('/v1/me/auth-links/00000000-0000-0000-0000-000000000000')
      .send({ status: 'INACTIVE' });
