...
import { makeCancelEnrollmentUseCase } from '@/use-cases/hr/benefit-enrollments/factories/make-cancel-enrollment-use-case';
...
export async function v1CancelEnrollmentController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    ...
    schema: {
      tags: ['HR - Benefits'],
      summary: 'Cancel benefit enrollment',
      description: 'Cancels a benefit enrollment',
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        await logAudit(request, {
          message: AUDIT_MESSAGES.HR.BENEFIT_ENROLLMENT_CANCEL,
