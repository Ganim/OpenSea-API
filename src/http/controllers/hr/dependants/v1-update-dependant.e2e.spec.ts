...
describe('Update Dependant (E2E)', () => {
  ...
  it('should update a dependant', async () => {
    ...
    const response = await request(app.server)
      .put(`/v1/hr/employees/${employeeId}/dependants/${dependantId}`)
    ...
    expect(response.body.dependant.name).toBe('Updated Name');
    expect(response.body.dependant.isIrrfDependant).toBe(true);
  });
