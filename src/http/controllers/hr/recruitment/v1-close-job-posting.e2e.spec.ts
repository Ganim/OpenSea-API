...
describe('Close Job Posting (E2E)', () => {
  let tenantId: string;
  ...
  it('should close an open job posting', async () => {
    ...
    // Create and publish a job posting first
    ...
    const response = await request(app.server)
      .patch(`/v1/hr/recruitment/job-postings/${jobPostingId}/close`)
      .set('Authorization', `Bearer ${token}`);

    expect(response.statusCode).toBe(200);
    expect(response.body).toHaveProperty('jobPosting');
    expect(response.body.jobPosting.status).toBe('CLOSED');
  });
