...
import { applicationResponseSchema, listApplicationsQuerySchema } from '@/http/schemas/hr/recruitment';
...
export async function v1ListApplicationsController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    ...
    schema: {
      ...
      response: { 200: z.object({ applications: z.array(applicationResponseSchema), total: z.number() }) },
      security: [{ bearerAuth: [] }],
    },
    handler: async (request, reply) => {
      ...
      const { applications, total } = await useCase.execute({ tenantId, ...filters });
      return reply.status(200).send({ applications: applications.map(applicationToDTO), total });
    },
  });
}
