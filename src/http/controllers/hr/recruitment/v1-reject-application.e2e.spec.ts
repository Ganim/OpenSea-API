...
describe('Reject Application (E2E)', () => {
  let tenantId: string;
  ...
  it('should reject an application', async () => {
    ...
    const candRes = await request(app.server)
      ...
      .send({
        ...
        email: `rej-app-${Date.now()}@test.com`,
        source: 'WEBSITE',
      });
    ...
    const response = await request(app.server)
      .patch(`/v1/hr/recruitment/applications/${applicationId}/reject`)
      .set('Authorization', `Bearer ${token}`)
      .send({ rejectionReason: 'Does not meet requirements' });
  ...
  it('should return 401 without token', async () => {
    const response = await request(app.server)
      .patch('/v1/hr/recruitment/applications/nonexistent-id/reject')
