...
import { makeRejectRequestUseCase } from '@/use-cases/hr/employee-requests/factories/make-reject-request-use-case';
import type { FastifyInstance } from 'fastify';
import type { ZodTypeProvider } from 'fastify-type-provider-zod';
import z from 'zod';

export async function v1RejectRequestController(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: 'POST',
    url: '/v1/hr/requests/:id/reject',
    preHandler: [
      verifyJwt,
      verifyTenant,
    ...
    schema: {
      ...
      description: 'Rejects a pending employee request with a reason',
      params: z.object({
        id: z.string().uuid(),
      }),
      body: z.object({
    ...
    handler: async (request, reply) => {
      ...
      try {
        const rejectRequestUseCase = makeRejectRequestUseCase();
        const { employeeRequest } = await rejectRequestUseCase.execute({
          tenantId,
          requestId,
        ...
        await logAudit(request, {
          message: AUDIT_MESSAGES.HR.EMPLOYEE_REQUEST_REJECT,
