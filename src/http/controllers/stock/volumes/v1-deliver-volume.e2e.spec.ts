...
describe('Deliver Volume (E2E)', () => {
  ...
  it('should mark a closed volume as delivered', async () => {
    ...
    const { volumeId } = await createVolumeE2E(app, {
      token,
      name: 'Volume para Entregar',
    });

    // Close the volume first
    ...
    expect(response.body.volume.status).toBe('DELIVERED');
    expect(response.body.volume.deliveredAt).not.toBeNull();
  });
});
