...
describe('Cancel Inventory Session (E2E)', () => {
  let tenantId: string;
  ...
  it('should cancel an open inventory session', async () => {
    ...
    const zone = await prisma.zone.create({
      data: {
        tenantId,
        code: `ZC${String(timestamp).slice(-2)}`,
        name: `Zone Cancel ${timestamp}`,
        warehouseId,
        structure: {},
      },
    });
    ...
    const bin = await prisma.bin.create({
      data: {
        tenantId,
        address: `CAN-${String(timestamp).slice(-4)}-01-A`,
        aisle: 1,
        shelf: 1,
        position: 'A',
        zoneId: zone.id,
    ...
    const response = await request(app.server)
      .patch(`/v1/stock/inventory-sessions/${sessionId}/cancel`)
      .set('Authorization', `Bearer ${token}`);
  ...
  it('should return 404 for non-existent session', async () => {
    ...
    const response = await request(app.server)
      .patch(
        '/v1/stock/inventory-sessions/00000000-0000-0000-0000-000000000000/cancel',
      )
      .set('Authorization', `Bearer ${token}`);
