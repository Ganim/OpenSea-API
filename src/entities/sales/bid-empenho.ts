...
export type BidEmpenhoStatusType =
  | 'EMITIDO'
...
export class BidEmpenho extends Entity<BidEmpenhoProps> {
  ...
  get empenhoNumber(): string {
    return this.props.empenhoNumber;
  }
  get type(): BidEmpenhoTypeEnum {
  ...
  get status(): BidEmpenhoStatusType {
    return this.props.status;
  }
  set status(value: BidEmpenhoStatusType) {
  ...
  set orderId(value: UniqueEntityID | undefined) {
  ...
  static create(
    props: Optional<BidEmpenhoProps, 'id' | 'createdAt' | 'status'>,
    id?: UniqueEntityID,
  ): BidEmpenho {
    return new BidEmpenho(
      {
        ...props,
        id: props.id ?? id ?? new UniqueEntityID(),
        status: props.status ?? 'EMITIDO',
