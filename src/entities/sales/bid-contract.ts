...
export type BidContractStatusType =
  | 'DRAFT_CONTRACT'
  | 'ACTIVE_CONTRACT'
  | 'SUSPENDED_CONTRACT'
  | 'COMPLETED_CONTRACT'
...
export class BidContract extends Entity<BidContractProps> {
  ...
  get status(): BidContractStatusType {
    return this.props.status;
  }
  set status(value: BidContractStatusType) {
    this.props.status = value;
    this.touch();
  }
  get signedDate(): Date | undefined {
  ...
  get remainingValue(): number {
    return this.props.remainingValue;
  }
  set remainingValue(value: number) {
    this.props.remainingValue = value;
    this.touch();
  }
  ...
  get maxRenewals(): number | undefined {
    return this.props.maxRenewals;
  }
  get renewalDeadline(): Date | undefined {
    return this.props.renewalDeadline;
  }
  get deliveryAddresses(): Record<string, unknown> | undefined {
    return this.props.deliveryAddresses;
  }
  get contractFileId(): UniqueEntityID | undefined {
    return this.props.contractFileId;
  }
  ...
  static create(
    props: Optional<
      BidContractProps,
      'id' | 'createdAt' | 'status' | 'renewalCount'
    >,
    id?: UniqueEntityID,
  ): BidContract {
    return new BidContract(
      {
        ...props,
        id: props.id ?? id ?? new UniqueEntityID(),
        status: props.status ?? 'DRAFT_CONTRACT',
        renewalCount: props.renewalCount ?? 0,
