...
export type BidHistoryActionType =
  | 'BID_CREATED'
  | 'BID_UPDATED'
  | 'BID_STATUS_CHANGED'
  | 'BID_PROPOSAL_CREATED'
  | 'BID_PROPOSAL_SENT'
  ...
  | 'BID_WON'
  | 'BID_LOST'
  | 'BID_CONTRACT_CREATED'
  | 'BID_EMPENHO_RECEIVED'
  | 'BID_ORDER_CREATED'
  | 'BID_AI_ANALYSIS'
  | 'BID_AI_DECISION'
  | 'BID_MANUAL_OVERRIDE';
...
export class BidHistory extends Entity<BidHistoryProps> {
  ...
  get action(): BidHistoryActionType {
  ...
  get performedByAi(): boolean {
    return this.props.performedByAi;
  }
  ...
  static create(
    props: Optional<
      BidHistoryProps,
      'id' | 'createdAt' | 'performedByAi' | 'isReversible'
    >,
    id?: UniqueEntityID,
  ): BidHistory {
    return new BidHistory(
      {
        ...
        performedByAi: props.performedByAi ?? false,
        isReversible: props.isReversible ?? false,
