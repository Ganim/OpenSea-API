...
export class CreatePipelineStageUseCase {
  ...
  async execute(
    ...
    // CRM pipelines only allow OPEN, WON, LOST stage types
    if (
      CRM_PIPELINE_TYPES.includes(pipeline.type) &&
      !CRM_VALID_STAGE_TYPES.includes(finalType)
    ) {
      throw new BadRequestError(
        `CRM pipelines only allow stage types: ${CRM_VALID_STAGE_TYPES.join(', ')}. Got: ${finalType}`,
      );
    }
