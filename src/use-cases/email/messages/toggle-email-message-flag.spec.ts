...
describe('ToggleEmailMessageFlagUseCase', () => {
  ...
  it('should flag a message', async () => {
    const account = await accountsRepository.findByAddress('user@example.com', 'tenant-1');
    const result = await messagesRepository.list({ tenantId: 'tenant-1', accountId: account!.id.toString(), limit: 1 });
    ...
    const updated = await messagesRepository.findById(message.id.toString(), 'tenant-1');
  ...
  it('should unflag a message', async () => {
    ...
    // Flag first
    await sut.execute({ tenantId: 'tenant-1', userId: 'user-1', messageId: message.id.toString(), isFlagged: true });
    // Then unflag
    await sut.execute({ tenantId: 'tenant-1', userId: 'user-1', messageId: message.id.toString(), isFlagged: false });
  ...
  it('should throw error if message not found', async () => {
    await expect(
      sut.execute({ tenantId: 'tenant-1', userId: 'user-1', messageId: 'invalid-id', isFlagged: true }),
  ...
  it('should throw error if user does not have read permission', async () => {
    ...
    await expect(
      sut.execute({ tenantId: 'tenant-1', userId: 'other-user', messageId: message.id.toString(), isFlagged: true }),
