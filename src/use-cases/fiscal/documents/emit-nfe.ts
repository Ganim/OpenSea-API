...
interface EmitNFeUseCaseRequest {
  tenantId: string;
  ...
  cfop?: string;
  items: EmitNFeItemRequest[];
  totalShipping?: number;
  additionalInfo?: string;
  orderId?: string;
}

interface EmitNFeUseCaseResponse {
...
export class EmitNFeUseCase {
  ...
  async execute(
    request: EmitNFeUseCaseRequest,
  ): Promise<EmitNFeUseCaseResponse> {
    ...
    if (request.items.length === 0) {
      throw new BadRequestError(
        'At least one item is required to emit an NF-e.',
      );
    }
    ...
    const cfop = request.cfop ?? fiscalConfig.defaultCfop;
    const naturezaOperacao =
      request.naturezaOperacao ?? fiscalConfig.defaultNaturezaOperacao;
    ...
    const documentItems = request.items.map((requestItem, index) => {
      ...
      const ipiBase = itemTotalPrice - itemDiscount;
      const ipiRate = requestItem.ipiRate ?? 0;
      ...
      totalTax += icmsValue + ipiValue + pisValue + cofinsValue;
    ...
    const fiscalDocument = FiscalDocument.create(
      {
        ...
        recipientIe: request.recipientIe,
        naturezaOperacao,
        cfop,
        totalProducts,
        totalDiscount,
    ...
    const documentEvent = FiscalDocumentEvent.create({
      ...
      description: emissionResult.success
        ? `NF-e ${nextNumber} authorized successfully`
        : `NF-e ${nextNumber} emission failed: ${emissionResult.errorMessage}`,
    ...
    if (emissionResult.success) {
      ...
      fiscalConfig.lastNfeNumber = nextNumber;
