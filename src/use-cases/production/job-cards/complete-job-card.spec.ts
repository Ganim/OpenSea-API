...
let sut: CompleteJobCardUseCase;

describe('CompleteJobCardUseCase', () => {
  beforeEach(() => {
    ...
    sut = new CompleteJobCardUseCase(jobCardsRepository);
  });

  it('should complete an in-progress job card', async () => {
    ...
    expect(jobCard.actualEnd).toBeDefined();
  });
  ...
  it('should throw error if job card is not IN_PROGRESS', async () => {
    ...
    // Try to complete a PENDING job card
    await expect(() =>
      sut.execute({
        id: created.id.toString(),
        tenantId: 'tenant-1',
