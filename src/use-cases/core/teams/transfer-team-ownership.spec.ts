...
describe('TransferTeamOwnershipUseCase', () => {
  beforeEach(() => {
    ...
    sut = new TransferTeamOwnershipUseCase(teamsRepository, teamMembersRepository);
  });
  ...
  it('should transfer ownership', async () => {
    ...
    const oldOwner = await teamMembersRepository.findByTeamAndUser(team.id, new UniqueEntityID('user-1'));
    const newOwner = await teamMembersRepository.findByTeamAndUser(team.id, new UniqueEntityID('user-2'));
