...
describe('RemoveTeamMemberUseCase', () => {
  beforeEach(() => {
    ...
    sut = new RemoveTeamMemberUseCase(teamsRepository, teamMembersRepository);
  });
  ...
  it('should reject removal by regular MEMBER', async () => {
    ...
    const target = await teamMembersRepository.create({ tenantId, teamId: team.id, userId: new UniqueEntityID('user-4') });
