...
describe('ListMyTeamsUseCase', () => {
  ...
  it('should list teams for the authenticated user', async () => {
    ...
    const { teams } = await sut.execute({ tenantId: 'tenant-1', userId: 'user-1' });
  ...
  it('should return empty array when user has no teams', async () => {
    const { teams, total } = await sut.execute({ tenantId: 'tenant-1', userId: 'user-1' });
  ...
  it('should return members count for each team', async () => {
    const tenantId = new UniqueEntityID('tenant-1');
    const team = await teamsRepository.create({ tenantId, name: 'Team A', slug: 'team-a', createdBy: new UniqueEntityID('user-1') });
