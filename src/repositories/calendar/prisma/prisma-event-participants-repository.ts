...
export class PrismaEventParticipantsRepository
  implements EventParticipantsRepository
{
  async create(data: CreateEventParticipantSchema): Promise<EventParticipant> {
    const raw = await prisma.eventParticipant.create({
      data: {
        ...
        userId: data.userId,
        role: (data.role as ParticipantRole) ?? 'GUEST',
        status: (data.status as ParticipantStatus) ?? 'PENDING',
      },
    });
  ...
  async updateStatus(
    ...
    const raw = await prisma.eventParticipant.update({
      where: { id },
      data: { status: status as ParticipantStatus, respondedAt: new Date() },
    });
    return eventParticipantPrismaToDomain(raw);
  }
